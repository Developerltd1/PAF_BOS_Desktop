...
namespace AttendanceScreen
{
    class FingerScannerClass
    {
        ...
        {
            Constants.ResultCode result = Constants.ResultCode.DP_DEVICE_FAILURE; //SET Reader Device Value to Fail
            result = currentReader.Open(Constants.CapturePriority.DP_PRIORITY_COOPERATIVE); //Checking Reader Device Able to Scan Or Not
            if (result != Constants.ResultCode.DP_SUCCESS)  //Checking Reader Device Not SUCCESS
            {
        ...
        public static bool StartCaptureAsync(Reader.CaptureCallback OnCaptured, Reader currentReader)
        {
            ...
            currentReader.On_Captured += new Reader.CaptureCallback(OnCaptured);   //Activate Reader to Scan Thumb

            // Call capture
            if (!CaptureFingerAsync(currentReader))   //Reader Capturing is Not Success
            {
                return false;
            }
        ...
        public static bool CaptureFingerAsync(Reader currentReader)
        {
            try
            {
                GetStatus(currentReader);   //Reader .Checking the Status of Reader, is Reader is ready Or Not 

                Constants.ResultCode captureResult = currentReader.CaptureAsync(Constants.Formats.Fid.ANSI, Constants.CaptureProcessing.DP_IMG_PROC_DEFAULT, currentReader.Capabilities.Resolutions[0]);  //Data Capturing Result From Reader
        ...
        {
            Constants.ResultCode result = currentReader.GetStatus();  //Reader BuiltIn Funcation for Showing ReaderStatus();

            if ((result != Constants.ResultCode.DP_SUCCESS)) //Reader Result is not SUCCESS
            {
                throw new Exception("" + result);
            }

            if ((currentReader.Status.Status == Constants.ReaderStatuses.DP_STATUS_BUSY)) //if Reader is Busy 50 miliSecond Wait
            {
                Thread.Sleep(50);  //Thread Class for Waiting
            }
            else if ((currentReader.Status.Status == Constants.ReaderStatuses.DP_STATUS_NEED_CALIBRATION))
            {
                currentReader.Calibrate();
            }
            else if ((currentReader.Status.Status != Constants.ReaderStatuses.DP_STATUS_READY))//if Reader is Not Ready
            {
